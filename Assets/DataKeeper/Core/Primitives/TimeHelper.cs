...
namespace ActionStage.Core.Primitives
{
    ...
    {
        public const int DAY_IN_SEC = 86400;
        ...
        public static string ToTimeLeftHHMMSSTextedFormat(int seconds)
        {
            var span = TimeSpan.FromSeconds(seconds);
            return $"Time Left: {span.Hours:00}:{span.Minutes:00}:{span.Seconds:00}";
        }
        ...
        public static string ToTimeLeftDDHHorHHMMorMMSSTextedFormat(int seconds)
        {
            var span = TimeSpan.FromSeconds(seconds);
            if (span.Days >= 1) return $"Time left: {span.Days}d {span.Hours:00}h";
            return span.Hours > 0 ? $"Time left: {span.Hours:00}h {span.Minutes:00}m" : $"Time left: {span.Minutes:00}m {span.Seconds:00}s";
        }
